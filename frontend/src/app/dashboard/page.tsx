    ...
    if (!isAuthenticated) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
                <GraduationCap className="w-16 h-16 text-uvci-purple mb-4" />
                <h1 className="text-2xl font-bold text-gray-900 mb-2">Accès Limité</h1>
                <p className="text-gray-500 text-center mb-6">Connectez-vous pour voir votre Dashboard Vision 360.</p>
                <Link href="/login" className="bg-uvci-purple text-white px-6 py-2 rounded-xl font-semibold">
                    Se connecter
                </Link>
            </div>
        );
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-4">
                            {events.length > 0 ? events.slice(0, 4).map((ev, i) => (
                                <div key={i} className="flex gap-4 items-start p-3 hover:bg-gray-50 rounded-2xl transition-colors border-l-4 border-uvci-purple">
                                    <div className="bg-purple-50 text-uvci-purple p-2 rounded-lg text-center min-w-[50px]">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-gray-900 line-clamp-1">{ev.title}</h4>
                                        <p className="text-xs text-gray-500">{ev.source}</p>
                                    </div>
                                </div>
                            )) : (
                                <p className="text-sm text-gray-400 text-center py-8">Aucun événement proche</p>
                            )}
                        </div>
                    </div>
